            ...
            <div class="galeria-brick galeria-brick--h" v-for="product in products">
                <a :href="'{{ url('/product/') }}'+'/'+product.slug">
                    ...
                    <div class="galeria_dimension">
                        <p v-for="size in product.product_format_sizes"  v-if="selectedLanguage == 'spanish'">@{{ size.size.width }}cm x @{{ size.size.height }}cm <span v-if="selectedCurrency == 'USD'">US$ @{{ number_format(price * exchangeRate, 2, ",", ".") }}</span><span v-else>COP @{{ number_format(price * exchangeRate, 2, ".", ",") }}</span></p>
                        <p v-for="size in product.product_format_sizes" v-if="selectedLanguage == 'english'">@{{ (size.size.width/2.54).toFixed(2) }}in x @{{ (size.size.height/2.54).toFixed(2) }}in <span v-if="selectedCurrency == 'USD'">US$ @{{ number_format(price * exchangeRate, 2, ",", ".") }}</span><span v-else>COP @{{ number_format(price * exchangeRate, 2, ".", ",") }}</span></p>
                    </div>
